...
def display_image_in_actual_size(img, dpi = 80)->'tuple[plt.Figure, plt.Axes]':
    height = img.shape[0]
    width = img.shape[1]
...
def display_snake(img, init_snake, result_snake):
    ...
    ax.scatter(init_snake[:, 0], init_snake[:, 1], s=2, c='r')
    # ax.plot(result_snake[:, 0], result_snake[:, 1], '-b', lw=2)
    ax.scatter(result_snake[:, 0], result_snake[:, 1], s=2, c='b')
