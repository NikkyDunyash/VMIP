...
class Func:
    ...
    def __call__(self, *args):
        if issubclass(type(args), Func):
            FuncCompose=types.new_class('FuncSum', bases=(Func,))
            FuncCompose.__call__=lambda obj, x: self(args(x))
            FuncCompose.grad=lambda obj, x: self.grad(args(x))*args.grad(x)
            return FuncCompose()
        else:
            return self._getval(*args)     
...
class Gaussian(Func):
    ...
    def _getval(self, *args):
        args=np.array(args) 
        return np.prod(np.exp(-(np.array(args-self.mean)**2)/(2*self.std**2)), axis=0)
    
    def grad(self, *args):
        args=np.array(args)
        return -np.array(args)/self.std**2*self._getval(*args)
